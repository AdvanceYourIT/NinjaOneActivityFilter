...
$btnSearch.Add_Click({
    ...
    if (-not [string]::IsNullOrWhiteSpace($deviceFilter) -and $deviceFilter -notmatch '^\d+$') {
        $deviceId = Resolve-DeviceIdByHostname -Hostname $deviceFilter -ClientID $script:OAuthConfig.ClientId -ClientSecret $script:OAuthConfig.ClientSecret
    ...
    try {
        $results = Get-Activities -Types $selectedTypes -After $afterDate -Before $beforeDate -DeviceId $deviceId -ClientID $script:OAuthConfig.ClientId -ClientSecret $script:OAuthConfig.ClientSecret
        ...
        foreach ($a in $script:AllResults) {
            ...
            $row['hostname'] = Resolve-DeviceHostname -DeviceId $row['deviceId'] -ClientID $script:OAuthConfig.ClientId -ClientSecret $script:OAuthConfig.ClientSecret
    ...
    if ($dlg.ShowDialog() -eq 'OK') {
        try {
            $script:AllResults | Select-Object @{N='Activity ID';E={ [string](Get-ObjectPropertyValue -InputObject $_ -PropertyName 'id') }},
                ...
                @{N='Hostname';E={ Resolve-DeviceHostname -DeviceId (Get-ObjectPropertyValue -InputObject $_ -PropertyName 'deviceId') -ClientID $script:OAuthConfig.ClientId -ClientSecret $script:OAuthConfig.ClientSecret }},
